        ...
        <v-card>
          <v-container>
            <v-form @submit.prevent="addEvent">
              ...
              <v-btn type="submit" color="primary" class="mr-4" @click.stop="dialog = false">Save</v-btn>
            </v-form>
          </v-container>
        </v-card>
      </v-dialog>
        ...
        >
          <v-card color="grey lighten-4" min-width="350px" flat>
            <v-toolbar :color="selectedEvent.color" dark>
              <v-btn fab text small @click="deleteEvent(selectedEvent.id)">
...
export default {
  ...
  methods: {
    async getEvents() {
      const snapshot = await db.collection("callEvent").get();
      const events = [];
    ...
    async addEvent(event) {
      if (this.name && this.start && this.end) {
        await db.collection("callEvent").add({
          name: this.name,
